...
초기 앱 제한은 전체 60회/일·IP별 10회/일입니다. 토스페이먼츠 테스트 결제를 서버에서 확인하면 같은 IP에 그날 10회가 한 번 추가됩니다. 테스트 결제는 실제 금액이 청구되지 않으며, 전체 60회 한도는 유지됩니다. 요금제 한도와 앱 제한은 다르며, 같은 Cloudflare 계정의 다른 서비스도 무료 AI 할당량을 함께 사용합니다. [배포 가이드와 비용 계산](docs/deployment.md)을 확인하세요.
