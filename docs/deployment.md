...
한도 화면의 토스페이먼츠 UI는 공식 문서용 테스트 클라이언트 키로 결제수단 선택과 인증 화면을 보여주는 데모입니다. 서버 결제 승인, 주문 금액 검증, 이용권 지급은 구현하지 않았으며 실제 결제와 대화 횟수 추가가 발생하지 않습니다. 라이브 판매 전에는 상점 계약, 자체 키, 서버 주문 원장과 승인 API, 웹훅·중복 지급 검증을 별도로 구현해야 합니다.
