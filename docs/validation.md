...
- `추가 결제하기`에서 토스페이먼츠 SDK v2와 공식 문서용 테스트 클라이언트 키로 결제수단·약관 UI를 표시. 금액 1,000원과 대화 10회는 UX 확인용 표시이며 승인 서버·실제 과금·횟수 지급은 없음.
- 테스트 키·데모 경계를 한국어/일본어 화면에 표시하고, 결제 인증 반환도 실제 지급 완료로 해석하지 않음. 라이브 전환에는 주문 원장, 서버 금액 검증, 승인 API, 웹훅과 중복 지급 방지가 필요.
