...
export const copy = {
  ja: {
    ...
    you: 'あなた', assistant: 'いっぽ', responseDemo: 'デモの定型応答', responseAI: 'AIの応答',
  ...
  ko: {
    ...
    you: '나', assistant: '잇포', responseDemo: '데모의 정해진 답변', responseAI: 'AI 답변',
...
export function getDemoReply(text: string, locale: Locale, turn: number): string {
  ...
  if (/疲|つら|しんど|지친|지쳤|피곤|힘들|tired/i.test(text)) {
    return locale === 'ja'
      ? 'お疲れさまでした。今日は少し、ペースをゆるめてもいいかもしれませんね。\n\n今できそうなら、お水をひとくち。何もしない時間をつくるのも、一つの選択です。'
      : '오늘도 수고했어요. 잠깐 속도를 늦춰봐도 좋겠어요.\n\n지금 괜찮다면 물을 한 모금 마셔볼까요. 아무것도 하지 않는 시간을 가지는 것도 하나의 선택이에요.';
  }
